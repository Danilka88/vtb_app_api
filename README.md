...
Эта архитектура обеспечивает высокую производительность, четкое разделение задач и масштабируемость.
...
3.  **Запустите приложение:**
    ...
    npm run dev
    ```
    Приложение UI будет доступно по адресу, указанному в консоли (обычно `http://localhost:3000`).

## 5. Как проверить работоспособность (workflow)

Благодаря архитектуре **Backend for Frontend (BFF)**, все данные, необходимые для работы UI, можно получить одним запросом. Это значительно упрощает взаимодействие и повышает производительность.

**Шаг 1: Получение всех агрегированных данных**

Отправьте POST-запрос на главный эндпоинт-агрегатор. Бэкенд самостоятельно соберет всю информацию из внутренних сервисов и вернет готовый для отображения объект `FinancialData`.

```bash
curl -X POST http://127.0.0.1:8000/api/v1/aggregator/all -H "Content-Type: application/json" -d '{}'
```

В ответе вы получите большой JSON-объект, содержащий все данные: от счетов и транзакций до персональных рекомендаций и настроек "умных" сервисов. Это подтверждает, что BFF-слой работает корректно и бэкенд готов к полноценной интеграции с UI.
...
- `POST /api/v1/aggregator/all`: **(BFF)** Получает все агрегированные финансовые данные для UI одним запросом.
