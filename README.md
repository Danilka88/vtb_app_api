...
-   **[Бэкенд] Переход на реальные данные в `ui_connector`:**
    -   **Задача:** Заменить моковые (mock) данные в `MCPService` и `LLMAgentService` на реальные вызовы к банковским API.
    -   **Почему это важно:** Это ключевой шаг для того, чтобы UI-компоненты, такие как "Умное списание" или "Анализ подписок",
      начали работать с настоящими данными пользователя.

-   **[Frontend] Интеграция UI с бэкендом:**
    -   **Задача:** Заменить моковый сервис `financialService.ts` на реальные HTTP-запросы к эндпоинтам `ui_connector`.
    -   **Почему это важно:** Чтобы UI-приложение стало полностью функциональным и отображало данные, полученные от бэкенда.
