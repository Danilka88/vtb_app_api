...
class TestABankPaymentWorkflow:
    ...
    def test_2_payment_consent_workflow(self, client: TestClient):
        ...
        # 2.2 Получение созданного согласия по ID для проверки (ПРОПУЩЕНО из-за предполагаемого требования mTLS)
    ...
    def test_3_payment_workflow(self, client: TestClient):
        ...
        print("\n--- Шаг 3: Создание и проверка статуса платежа (Abank) ---")
        ...
        response_payment = client.post(
            f"/api/v1/payments/abank/create",
        ...
        TestABankPaymentWorkflow.payment_id = details.get("paymentId") or details.get("data", {}).get("paymentId")
        ...
        response_status = client.get(
            f"/api/v1/payments/abank/{self.payment_id}/status",
    ...
    def test_4_revoke_payment_consent(self, client: TestClient):
        ...
        """
        assert self.payment_consent_id, "Не удалось получить ID платежного согласия."
        print("\n--- Шаг 4: Отзыв платежного согласия (Abank) ---")

        # 4.1 Отзыв согласия
        print(f"\n4.1. Отзыв платежного согласия по ID: {self.payment_consent_id}...")
        response_revoke = client.delete(f"/api/v1/payments/payment-consents/{self.payment_consent_id}?bank_name=abank&user_id={USER_ID}")
        ...
        response_get_revoked = client.get(f"/api/v1/payments/payment-consents/{self.payment_consent_id}?bank_name=abank&user_id={USER_ID}")
