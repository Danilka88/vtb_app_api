...
def test_create_account(init_bank_tokens): # Добавляем фикстуру как аргумент
    ...
    print("\n--- Тест: Создание счета для ABank ---")

    request_data = {
        "account_type": "Personal",
        "initial_balance": 1000.0
    }
    response = client.post("/api/v1/data/accounts", params={"bank_name": "abank"}, json=request_data) # Изменено bank_name
...
def test_get_account_details(create_consent_fixture): # Добавляем фикстуру как аргумент
    ...
    """
    global TEST_CONSENT_ID
    print("\n--- Тест: Получение деталей счета для ABank ---")

    # 1. Создание согласия на доступ к счетам
    print("\n1.1. Создание согласия на доступ к счетам для ABank...")
    consent_request = {
